...
/**
 * @author Casey
 * A Game Manager Interface connects all of the pieces of the model. Most logic that occurs will happen in the
 * Game Manager and the Game Manager will get whatever information it needs, as it needs it, from the different
 * model classes. The Game Manager handles game initialization as well as all other actions that occur.
 */
public interface IGameManager {
	/**
	 * Called at the start of a game. Calls the following helper functions initialize the game:
	 * setCureIndicators(), setInfectionRate(), setOutbreakIndicator(), shuffleInfectionDeck(), drawAndInfect(),
	 * assignPlayerRoles(), dealCards(), setDifficultyLevel(), preparePlayerDeck()
	 */
	public void initializeGame();
	
	/**
	 * Initializes the cure indicator; this will likely occur by simply initializing a class
	 * that tracks the cure indicator.
	 */
	public void setCureIndicators();
	
	/**
	 * Sets the infection rate; this will likely occur by simply initializing the class that tracks
	 * the infection rate.
	 */
	public void setInfectionRate();
	
	/**
	 * Sets the outbreak indicator; this will likely occur by simply initializing the class that tracks
	 * the outbreak indicator.
	 */
	public void setOutbreakIndicator();
	
	/**
	 * Shuffles the 48 infection cards in the infection card deck
	 */
	public void shuffleInfectionDeck();
	
	/**
	 * Draws three cards from the infection pile, then places 3 disease cubes on the 3 drawn cities. Draws three more
	 * cards from the infection pile and places 2 disease cubes on the 3 drawn cities. Draws three more cards and
	 * places 1 disease cube on the 3 drawn cities.
	 */
	...
	/**
	 * Calls shuffleCityAndEventCards(), then gives the appropriate number of cards to each player, based on how many
	 * players are in the game: 2 players = 4 cards eacy, 3 players = 3 cards each, 4 players = 2 cards each
	 */
	public void dealCards();
	
	/**
	 * Sets the number of epidemic cards based on the provided difficulty level: "Normal" = 4 epidemic cards,
	 * "Hard" = 5 epidemic cards, "Insane" = 6 epidemic cards (your funeral)
	 * @param difficulty
	 */
	public void setDifficultyLevel(String difficulty);
	
	/**
	 * Divides the player deck into equally sized piles; the number of equal sized piles equals the number of infection
	 * cards being used. Then one epidemic card is placed on each pile and each pile is shuffled individually. Then the
	 * piles are stacked on top of each other.
	 */
	public void preparePlayerDeck();
	
	/**
	 * Draws two cards from the player deck. If there are less than 2 cards in the pile, the game is lost. If after drawing 
	 * 2 cards the player has more than 7 cards, the player must discard two cards. If one or 2 epidemic cards are drawn, 
	 * the steps on the epidemic cards are followed. Event cards may be played between epidemic cards.
	 */
	public void drawTwoFromPlayerDekc();
	
	/**
	 * Draws an infection card and infects the city that is drawn, 1 cube per city. If infecting the city results in more 
	 * than 3 cubes of one color on any city, perform DiseaseOutbreak is then called on that city.
	 */
	public void infectCities();
	
	/**
	 * The outbreak counter is incremented by 1. If the outbreak counter reaches 8, the game is lost. The provided city 
	 * retains 3 cubes of the provided color. Any city connected to the provided city is infected with 1 disease cube of 
	 * the provided color. If any of the connected cities already have 3 disease cubes of the provided color, a disease
	 * outbreak occurs at that city with the provided color, resulting in a chain outbreak. A city may only experience
	 * an outbreak once per chain outbreak. (Cities should have some hasOutbroken boolean or something)
	 * @param city The city where the outbreak is occurring; this city's neighbors get infected
	 * @param color The color of disease that will spread to connected cities
	 */
	public void performDiseaseOutbreak(String city, String color);
	
	/**
	 * Called when an epidemic card is drawn. Increments the infection rate by 1. Draws the bottom card of the infection
	 * deck and infects the drawn city with 3 disease cubes of that city's color. If that city has any number of disease
	 * cubes of that city's color, that city retains 3 of those cubes and an outbreak occurs in that city.
	 */
	public void performEpidemic();
	
	///////////////////////////Model Facade Functions//////////////////////////
    /**
     ...
     * @return A Map mapping a player's ID to the city where their pawn is.
     */
